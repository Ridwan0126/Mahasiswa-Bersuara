...
if (isset($_POST['filter-tgl'])) {
  ...
  $approve  = $conn->query("SELECT * FROM laporan,mahasiswa,kategori,status_laporan where status_laporan.status='approve' and status_laporan.id_status=laporan.id_status and laporan.nim=mahasiswa.nim and laporan.id_kategori=kategori.id_kategori AND laporan.created_at BETWEEN '$tgl_mulai' AND DATE_ADD('$tgl_selesai', INTERVAL 1 DAY)");
  $unapprove = $conn->query("SELECT * FROM laporan,mahasiswa,kategori,status_laporan where status_laporan.status='unapprove' and status_laporan.id_status=laporan.id_status and laporan.nim=mahasiswa.nim and laporan.id_kategori=kategori.id_kategori AND laporan.created_at BETWEEN '$tgl_mulai' AND DATE_ADD('$tgl_selesai', INTERVAL 1 DAY)");
