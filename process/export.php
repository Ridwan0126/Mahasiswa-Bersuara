...
if (isset($_POST['export-file'])) {
  ...
  $query = "SELECT * FROM laporan AS lp LEFT JOIN mahasiswa AS mahasiswa ON lp.nim = mahasiswa.nim LEFT JOIN kategori AS kat ON lp.id_kategori = kat.id_kategori LEFT JOIN status_laporan AS sl ON lp.id_status = sl.id_status";
  $result = $conn->query($query);

  $worksheet->setCellValue('A1', 'No');
  $worksheet->setCellValue('B1', 'Keluhan');
  $worksheet->setCellValue('C1', 'Kategori');
  $worksheet->setCellValue('D1', 'nim');
  ...
  foreach ($result as $laporan) {
    ...
    $worksheet->setCellValue('D' . $column, $laporan['nim']);
    $worksheet->setCellValue('E' . $column, $laporan['nama']);
    $worksheet->setCellValue('F' . $column, $laporan['prodi']);
